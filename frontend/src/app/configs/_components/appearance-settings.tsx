  ...
  return (
    <div className="space-y-5">
      <SettingsGroup
        ...
        description="Lite hides infra tabs for agent-only desktop use; full exposes vLLM controls."
      >
        <SettingsRow
          ...
          control={
            <SegmentedOptions
              value={liteMode ? "lite" : "full"}
              options={[
                { id: "lite", label: "Lite (Agent)" },
                { id: "full", label: "Full (Infra)" },
              ]}
              onChange={(value) => setLiteMode(value === "lite")}
            />
          }
          status={
            <StatusPill tone={liteMode ? "info" : "good"}>
              {liteMode ? "lite" : "full"}
            </StatusPill>
          }
        />
